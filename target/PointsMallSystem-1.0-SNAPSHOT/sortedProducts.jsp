...
<div class="container">
    <%
        ...
        /*int pageSize = 25;
        int pageNumber = (int) Math.ceil((double) allProducts.size() / pageSize);
        int currentPage = request.getParameter("page") != null ? Integer.parseInt(request.getParameter("page")) : 1;
*/

    // 获取当前页需要显示的产品数据
   /* List<Product> currentProducts = ProductService.getProductsByPage(allProducts,currentPage,pageSize);*/
...
<div class="pagination">
    <% if (currentPage > 1) { %>
    <a href="sortedProducts.jsp?page=<%= currentPage - 1 %>">上一页</a>
    <% } %>

    <% for (int i = 1; i <= pageNumber; i++) { %>
    <% if (i == currentPage) { %>
    <a class="current" href="sortedProducts.jsp?page=<%= i %>"><%= i %></a>
    <% } else { %>
    <a href="sortedProducts.jsp?page=<%= i %>"><%= i %></a>
    <% } %>
    <% } %>

    <% if (currentPage < pageNumber) { %>
    <a href="sortedProducts.jsp?page=<%= currentPage + 1 %>">下一页</a>
    <% } %>
</div>--%>
